	...
	<div id="content" class="conteudo update page" role="main">
			...
			if ( !empty($_POST)) {
			    ...
			    if ($valid) {
			        ...
			        $sql = "UPDATE webinars set nome = ?, sobrenome = ?, instituicao = ?, titulo = ?, data = ?, horario = ?, resumo = ?, apresentacao = ?, video = ?, foto = ? WHERE id = ?";
			        $q = $pdo->prepare($sql);
			        $uploadOK = true;
			        if (!empty($foto) && $foto != $_POST['old_foto']) {
		    ...
		    <section>
		        <h1>Editar</h1>
		            ...
		            <form action="<?php echo get_bloginfo('template_url') . '/webinars_update.php?id=' . $id ?>" method="post" enctype="multipart/form-data">
						<!-- Nome -->
