	...
	<div id="content" class="conteudo create page" role="main">
			...
			if ( !empty($_POST)) {
			    ...
			    if ($valid) {
			        ...
			        $sql = "INSERT INTO lunchtalks (nome, sobrenome, instituicao, titulo, data, horario, resumo, apresentacao, video, foto) VALUES (?,?,?,?,?,?,?,?,?,?)";
			        $q = $pdo->prepare($sql);
			        $uploadOK = true;
			        if (!empty($foto)) {
			        ...
			        $sql_log = "INSERT INTO log (wp_username, datetime, action, page, resumo) VALUES (?, now(), 'INSERT', 'LUNCHTALKS', ?)";
		    ...
		    <section>
		        <h1>Criar</h1>
		            ...
		            <form action="<?php echo get_bloginfo('template_url') . '/lunchtalks_create.php' ?>" method="post" enctype="multipart/form-data">
						<!-- Nome -->
