	...
	<div id="content" class="conteudo create page" role="main">
			...
			if ( !empty($_POST)) {
			    ...
			    if ($valid) {
			        ...
			        $sql = "INSERT INTO hotspot (nome, sobrenome, instituicao, titulo, data, horario, resumo, apresentacao, video, foto) VALUES (?,?,?,?,?,?,?,?,?,?)";
			        $q = $pdo->prepare($sql);
			        $uploadOK = true;
			        if (!empty($foto)) {
			        ...
			        $sql_log = "INSERT INTO log (wp_username, datetime, action, page, resumo) VALUES (?, now(), 'INSERT', 'HOTSPOT', ?)";
		    ...
		    <section>
		        <h1>Criar</h1>
		            ...
		            <form action="<?php echo get_bloginfo('template_url') . '/hotspot_create.php' ?>" method="post" enctype="multipart/form-data">
						<!-- Nome -->
