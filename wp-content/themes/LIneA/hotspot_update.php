	...
	<div id="content" class="conteudo update page" role="main">
			...
			if ( !empty($_POST)) {
			    ...
			    if ($valid) {
			        ...
			        $sql = "UPDATE hotspot set nome = ?, sobrenome = ?, instituicao = ?, titulo = ?, data = ?, horario = ?, resumo = ?, apresentacao = ?, video = ?, foto = ? WHERE id = ?";
			        $q = $pdo->prepare($sql);
			        $uploadOK = true;
			        if (!empty($foto) && $foto != $_POST['old_foto']) {
			        ...
			        $sql_log = "INSERT INTO log (wp_username, datetime, action, page, resumo) VALUES (?, now(), 'UPDATE', 'HOTSPOT', ?)";
			    ...
			    $pdo = Database::connect();
			    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			    $sql = "SELECT * FROM hotspot where id = ?";
			    $q = $pdo->prepare($sql);
			    $q->execute(array($id));
		    ...
		    <section>
		        <h1>Editar</h1>
		            ...
		            <form action="<?php echo get_bloginfo('template_url') . '/hotspot_update.php?id=' . $id ?>" method="post" enctype="multipart/form-data">
						<!-- Nome -->
						...
						<div class="grupo-input">
		                  ...
		                  <a class="btn" href="/hotspot/">Voltar</a>
		                </div>
		            </form>
		    </section>
		    </main>
